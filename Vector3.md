...
| Свойство        | Значение (x, y, z) | Описание направления                              |
|-----------------|--------------------|--------------------------------------------------|
| Vector3.zero   | (0, 0, 0)          | Начало координат или отсутствие значения.       |
| Vector3.one    | (1, 1, 1)          | Единичный масштаб для всех осей.                |
| Vector3.forward| (0, 0, 1)          | Направление вдоль положительной оси Z.          |
| Vector3.back   | (0, 0, -1)         | Направление вдоль отрицательной оси Z.          |
| Vector3.up     | (0, 1, 0)          | Направление вдоль положительной оси Y.          |
| Vector3.down   | (0, -1, 0)         | Направление вдоль отрицательной оси Y.          |
| Vector3.right  | (1, 0, 0)          | Направление вдоль положительной оси X.          |
| Vector3.left   | (-1, 0, 0)         | Направление вдоль отрицательной оси X.          |
...
if (dot > 0.8f)
{
	...
	```
4. `Cross` - **векторное** произведение. Возвращает вектор, **перпендикулярный** двум другим. 
   Используется для:
	   - вычисления **оси** **вращения**
	   - физики
	   - более сложной 3D-математике
5. `Angle` - возвращает угол между векторами **в градусах**.
   ```cs
   ...
   Используется для:
	   ...
	   - проверка угла атаки
	- AI
6. `Lerp` - [[Интерполяция между точками|интерполирует]] между точками.
	```cs
...
6. `Reflect` - **отражает** вектор **от** **поверхности**.
   ```cs
   ...
   - отражения лучей
   - физики
6. `ClampMagnitude` - ограничивает **длину** вектора.
   ```cs
