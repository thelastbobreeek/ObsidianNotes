...
4. `Dot`(скалярное произведение) - считает, в какие стороны смотрят объекты относительно друг друга:
	```cs
...
if (dot > 0.8f)
{
    Debug.Log("Цель почти прямо перед нами");
}
	```
5. `Cross` - векторное произведение. Возвращает вектор, перпендикулярны двум другим. 
   Используется для:
	   - вычисления оси вращения
	   - физики
	   - более срочной 3D-математике
6. `Angle` - возвращает угол между векторами в градусах.
   ```cs
   ...
   Используется для:
	   ...
	   - проверка угла атаки
	- AI
7. `Lerp` - [[Интерполяция между точками|интерполирует]] между точками.
	```cs
	...
	- камеры
	- анимаций
7. `MoveTowards` - двигает к цели, но не превышает `maxDelta`. Никогда не перелетает её. 
   ...
   Отлично подходит для:
	...
	Часто лучше, чем `Lerp` для движения.
8. 